...
export function useLocationsList({ departmentIds, search, status, sortBy, sortDirection, pageSize }: LocationFilterState) {
    ...
    const { data, isLoading, error, isFetching, refetch, fetchNextPage, isFetchingNextPage, hasNextPage } = useInfiniteQuery({
        ...locationQueryOptions.getLocationsInfiniteOptions({ departmentIds, search: search, status: status, sortBy: sortBy, sortDirection: sortDirection, pageSize: pageSize })
    });
