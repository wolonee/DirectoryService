...
export function useLocationsList({ departmentIds, search, isActive, sortBy, sortDirection, pageSize }: LocationFilterState) {
    ...
    const { data, isLoading, error, isFetching, refetch, fetchNextPage, isFetchingNextPage, hasNextPage } = useInfiniteQuery({
        ...locationQueryOptions.getLocationsInfiniteOptions({ departmentIds, search: search, isActive: isActive, sortBy: sortBy, sortDirection: sortDirection, pageSize: pageSize })
    });
