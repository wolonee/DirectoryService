...
import { GetPositionDto, GetPositionsRequest, GetPositionsResponse } from "./types";
...
export const positionApi = {
  getPositions: async (
    request: GetPositionsRequest): Promise<PaginationResponse<GetPositionDto>> => {
...
export const positionQueryOptions = {
  ...
  getLocationsInfiniteOptions: ({ pageSize, sortBy, sortDirection }: { pageSize: number; sortBy: string; sortDirection: string }) => {
    return infiniteQueryOptions({
      queryKey: [positionQueryOptions.baseKey, "infinite", { pageSize, sortBy, sortDirection }],
      queryFn: ({ pageParam }) => 
        positionApi.getPositions({ pagination: { page: pageParam, pageSize }, sortBy, sortDirection }),
      initialPageParam: 1,
