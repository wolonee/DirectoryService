...
**Mutations & backend errors.** A mutation hook wraps `useMutation` (`const mutation = useMutation(...)`), invalidates the entity `baseKey` in `onSettled`, toasts in `onError`/`onSuccess`, and returns `{ <verb>: mutation.mutate, isPending, error, commonError, resetError }` — `error` is an `EnvelopeError`, `commonError` a non-Envelope failure. Backend errors are `ApiError { code, message, invalidField?, type }` inside `Envelope.errors`; use `EnvelopeError.fieldErrors` (those with `invalidField`) to render **field-level** messages next to the offending input and the rest as **operation-level** messages, mapping known `code`s to human text.
