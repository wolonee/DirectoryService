...
- **Writes** go through EF Core + the domain. Domain entities (`Location`, `Department`, `Position`) have private constructors and static `Create(...)` factories returning `Result`, with value objects (`DepartmentName`, `DepartmentIdentifier`, …) whose payload is `.Value`. Deletion is soft (`IsDeleted` + `DeletedAt`), plus an `IsActive` flag; entities expose `SoftDelete()` and `Restore()` mutators, and restore is a `POST /{id}/restore` command per aggregate (`RestoreLocation`/`RestoreDepartment`/`RestorePosition`).
- **Reads** use **Dapper raw SQL** via `IDbConnectionFactory` (it returns the DbContext's own `IDbConnection`, so use a plain `using`/no-`using` — `await using` won't compile). Single-object reads (e.g. `GetDepartmentByIdHandler`) may use EF Core LINQ instead; Dapper is the pattern for **paged lists**. **There is no global soft-delete query filter**, so every read must add `WHERE is_deleted = false` itself or deleted rows leak. List endpoints take a `Status` string (`all|active|inactive|archived`) — validated against an allow-list — that maps to `is_active`/`is_deleted` (`archived` = only soft-deleted; the rest exclude deleted).
